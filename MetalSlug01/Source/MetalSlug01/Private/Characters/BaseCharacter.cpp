...
ABaseCharacter::ABaseCharacter()
{
	...
	// 注: MaxHealth/CurrentHealth/bIsDead 字段已下沉到 HealthComponent
	// 这里只初始化 Character 自己仍保留的字段 (MaxEnergy 等)
	MaxEnergy = 100.0f;
	CurrentEnergy = MaxEnergy;

	// 同步到 Component (改造: 业务逻辑已抽离, Component 也要拿到初值)
	// 【2026-06-15 简化】: 不再传 CurrentHealth,Component 内部 MaxHealth 即满血
	if (HealthComponent)
	{
		HealthComponent->InitializeHealth(100.0f);
	}
	if (EnergyComponent)
	{
		EnergyComponent->InitializeEnergy(MaxEnergy, CurrentEnergy);
	}
...
void ABaseCharacter::BeginPlay()
{
	...
	// 【2026-06-15 新增】: 订阅 HealthComponent 事件
	...
	if (HealthComponent)
	{
		// 使用 AddUniqueDynamic 避免重复绑定 (BeginPlay 可能被多次调用)
...
void ABaseCharacter::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	if (UWorld* World = GetWorld())
	{
		...
		// 清理死亡流程相关的 timer
		// 这三个 timer 在快速死亡-重生-再死亡场景下会产生残留,必须显式清理
		TM.ClearTimer(WeaponDestroyTimerHandle);
		TM.ClearTimer(DissolveTimerHandle);
 ...
 *
 * 能量改变时的客户端回调
 * 【2026-06-15 重构】: 改为调用 TryResolveHUDWidget 后直接更新能量
 */
...
void ABaseCharacter::OnKill(ABaseCharacter* KilledCharacter)
{
	...
	if (HealthComponent)
	{
		HealthComponent->AddHealth(HealthRewardPerKill);
	}
...
void ABaseCharacter::Multicast_Die_Implementation()
{
	...
	// 开启 10 秒倒计时
	// 【2026-06-15 修复】: 原为局部 FTimerHandle,改为成员以便精确管理
...
void ABaseCharacter::RefreshHUDFromCurrentState()
{
	...
	// 能量 (仍由 BaseCharacter 持有字段, 后续若下沉到 EnergyComponent 一并改)
	GameHUDWidget->UpdateEnergy(CurrentEnergy, MaxEnergy);
	GameHUDWidget->UpdateEnergyText(FMath::CeilToInt(CurrentEnergy), FMath::CeilToInt(MaxEnergy));
