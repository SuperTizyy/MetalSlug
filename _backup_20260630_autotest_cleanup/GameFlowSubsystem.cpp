...
// 引入自动测试房间协调器（bAutoCreateTestRoom=true 时启动自动创房/加房）
...
void UGameFlowSubsystem::BootToLogin()
{
	...
	// 【大厂架构 - 文件落盘探针 2026.06.30】绕过 PIE 早期 stdout 丢失问题
	{
		...
		const FString Line = FString::Printf(
			TEXT("[%s][PID=%u] [GameFlow] BootToLogin 入口, CurrentState=PreLogin\n"),
	...
	// 【大厂架构 - 文件落盘探针 2026.06.30】bSkipLoginDirectToLobby 入口探针
	{
		const FString LogFilePath = FPaths::ProjectSavedDir() / TEXT("AutoTestRoom.log");
		const bool bSkip = TestSettings && TestSettings->bSkipLoginDirectToLobby;
		const bool bAutoRoom = TestSettings && TestSettings->bAutoCreateTestRoom;
		const FString Line = FString::Printf(
			TEXT("[%s][PID=%u] [GameFlow] TestSettings: bSkipLoginDirectToLobby=%d, bAutoCreateTestRoom=%d\n"),
			*FDateTime::Now().ToString(),
			FPlatformProcess::GetCurrentProcessId(),
			bSkip ? 1 : 0,
			bAutoRoom ? 1 : 0);
	...
	if (TestSettings && TestSettings->bSkipLoginDirectToLobby)
	{
		...
		// 2. 先切到 MainLobby 让 UI 可见（即使自动房间协调正在运行，用户也能看到大厅）
		//    切状态不跳地图：MainLobby 对应 LANRoom 面板，留在 L_Login 常驻地图
		TransitToState(EMatchState::MainLobby);

		// 3. 如果开启了 bAutoCreateTestRoom，启动自动测试房间协调器
		//    行为 (按启动顺序指定方案):
		//      - Index=0: 第一个客户端, 1s 后创建 AutoTestRoom 房主身份
		//      - Index>0: 后续客户端, 1.5s/2.0s... 后开始搜索 AutoTestRoom, 找到则加入
		//    设计: PIE NumberOfClients=3 时, GetGameInstance()->GetIndex()
		//          按 0/1/2 启动顺序稳定分配, 不需要竞态
		if (TestSettings->bAutoCreateTestRoom)
		{
			UE_LOG(LogGameFlow, Log,
				TEXT("[GameFlow][自动测试] bAutoCreateTestRoom=true，启动自动房间协调..."));

			// 【大厂架构 - 文件落盘探针 2026.06.30】
			{
				const FString LogFilePath = FPaths::ProjectSavedDir() / TEXT("AutoTestRoom.log");
				const FString Line = FString::Printf(
					TEXT("[%s][PID=%u] [GameFlow] bAutoCreateTestRoom=true, 准备获取 AutoTestRoomCoordinatorSubsystem\n"),
					*FDateTime::Now().ToString(),
					FPlatformProcess::GetCurrentProcessId());
				FFileHelper::SaveStringToFile(
					Line,
			...
			if (UAutoTestRoomCoordinatorSubsystem* AutoTestCoordinator =
				GetGameInstance()->GetSubsystem<UAutoTestRoomCoordinatorSubsystem>())
			{
				// 使用 Project Settings 中的房间名称（默认 "AutoTestRoom"）
				AutoTestCoordinator->StartAutoTestRoom(TestSettings->AutoTestRoomName);
			}
			else
			{
				UE_LOG(LogGameFlow, Error,
					TEXT("[GameFlow][自动测试] AutoTestRoomCoordinatorSubsystem 不可用"));

				// 【大厂架构 - 文件落盘探针 2026.06.30】
				{
					const FString LogFilePath = FPaths::ProjectSavedDir() / TEXT("AutoTestRoom.log");
					const FString Line = FString::Printf(
						TEXT("[%s][PID=%u] [GameFlow] ERROR: AutoTestRoomCoordinatorSubsystem 不可用!\n"),
						*FDateTime::Now().ToString(),
						FPlatformProcess::GetCurrentProcessId());
					FFileHelper::SaveStringToFile(
						Line,
